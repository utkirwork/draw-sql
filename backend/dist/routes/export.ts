...
const sqlExportService = new SQLExportService();
 ...
 */
router.post('/sql/:diagramId', authMiddleware, async (req: AuthenticatedRequest, res) => {
    try {
        ...
        const tables = transformedTables.map(table => ({
            ...
            columns: table.columns.map(col => {
                ...
                if (col.defaultValue) column.default_value = col.defaultValue;
                if (col.comment) column.description = col.comment;
        ...
        const relationships = (diagramData.relationships || []).map((rel: any) => ({
            id: rel.id || '',
            diagram_id: diagramId!,
        ...
        // Generate SQL schema
        const sqlSchema = sqlExportService.generateSQLSchema(
            tables, 
            relationships, 
            databaseType, 
            schemaName
        ...
        logger.error('Error exporting SQL', {
 ...
 */
router.post('/pdf/:diagramId', authMiddleware, async (req: AuthenticatedRequest, res) => {
    try {
        const { diagramId } = req.params;
        ...
        const pdfBuffer = await pdfExportService.generatePDFDocumentation(
            documentTitle || diagram.name || 'Database Schema Documentation',
            diagram.description || '',
            tables,
            relationships,
            databaseType,
