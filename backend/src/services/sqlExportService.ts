...
export class SQLExportService {
     ...
     */
    private generateCreateTableSQL(table: DiagramTable, databaseType: DatabaseType, schemaName: string): string {
     ...
     */
    private generateColumnDefinition(column: TableColumn, databaseType: DatabaseType): string {
        const parts: string[] = [];
     ...
     */
    private mapDataType(column: TableColumn, databaseType: DatabaseType): string {
    ...
    private mapPostgreSQLType(dataType: string, length?: number, precision?: number, scale?: number): string {
        switch (dataType) {
            case 'varchar':
    ...
    private mapMySQLType(dataType: string, length?: number, precision?: number, scale?: number): string {
        switch (dataType) {
            case 'varchar':
    ...
    private mapSQLServerType(dataType: string, length?: number, precision?: number, scale?: number): string {
        switch (dataType) {
            case 'varchar':
                return length ? `VARCHAR(${length})` : 'VARCHAR(255)';
            case 'char':
    ...
    private mapMariaDBType(dataType: string, length?: number, precision?: number, scale?: number): string {
     ...
     * Generate indexes
     */
    private generateIndexes(tables: DiagramTable[], databaseType: DatabaseType, schemaName: string): string[] {
        const statements: string[] = [];
     ...
     */
    private generateCommentSQL(table: DiagramTable, databaseType: DatabaseType, schemaName: string): string {
     ...
     */
    private quoteIdentifier(identifier: string, databaseType: DatabaseType): string {
        switch (databaseType) {
            case 'postgresql':
                return `"${identifier}"`;
            case 'mysql':
     ...
     */
    private escapeString(str: string, databaseType: DatabaseType): string {
