...
import { DiagramTable, TableColumn, TableRelationship, DatabaseType } from '../types';

export class PDFExportService {
    /**
     * Generate PDF documentation from diagram data
     */
    public async generatePDFDocumentation(
        diagramName: string,
...
${this.generateSQLPreview(tables, relationships, databaseType, schemaName)}
     ...
     */
    private generateTableDocumentation(table: DiagramTable, index: number): string {
     ...
     */
    private generateRelationshipDocumentation(rel: TableRelationship): string {
        const typeLabels = {
     ...
     */
    private generateRelationshipDiagrams(tables: DiagramTable[], relationships: TableRelationship[]): string {
     ...
     */
    private generateTwoTableDiagram(fromTable: DiagramTable, toTable: DiagramTable, relationship: TableRelationship): string {
     ...
     */
    private generateTableSVG(table: DiagramTable, x: number, y: number, width: number, height: number, relationship: TableRelationship, direction: 'from' | 'to'): string {
     ...
     */
    private getRelevantColumnsForRelationship(table: DiagramTable, relationship: TableRelationship, direction: 'from' | 'to'): TableColumn[] {
     ...
     */
    private getLinkedColumns(table: DiagramTable, relationships: TableRelationship[]): TableColumn[] {
        if (!table.columns) return [];
        ...
        return table.columns.filter(column => 
            linkedColumnNames.has(column.name) || 
            column.is_primary_key || 
            column.is_foreign_key
        );
    }
